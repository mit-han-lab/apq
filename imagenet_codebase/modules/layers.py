...
class ConvLayer(My2DLayer):
    ...
    def __init__(self, in_channels, out_channels,
                 ...
                 use_bn=True, act_func='relu', dropout_rate=0, ops_order='weight_bn_act', **kwargs):
...
class PoolingLayer(My2DLayer):
    ...
    def __init__(self, in_channels, out_channels,
                 ...
                 use_bn=False, act_func=None, dropout_rate=0, ops_order='weight_bn_act', **kwargs):
...
class MBInvertedConvLayer(MyModule):
    ...
    def __init__(self, in_channels, out_channels,
                 kernel_size=3, stride=1, expand_ratio=6, mid_channels=None, act_func='relu6', **kwargs):
...
class QConvLayer(My2DLayer):

    def __init__(self, in_channels, out_channels,
                 kernel_size=3, stride=1, dilation=1, groups=1, bias=False, has_shuffle=False,
                 use_bn=True, act_func='relu', dropout_rate=0, ops_order='weight_bn_act',
                 w_bit=-1, a_bit=-1, half_wave=True, **kwargs):
        ...
        super(QConvLayer, self).__init__(in_channels, out_channels, use_bn, act_func, dropout_rate, ops_order)
    ...
    def weight_op(self):
        ...
        weight_dict['conv'] = QConv2d(
            self.in_channels, self.out_channels, kernel_size=self.kernel_size, stride=self.stride, padding=padding,
            dilation=self.dilation, groups=self.groups, bias=self.bias, w_bit=self._w_bit, a_bit=self._a_bit, half_wave=self._half_wave
        )
        if self.has_shuffle and self.groups > 1:
            weight_dict['shuffle'] = ShuffleLayer(self.groups)
    ...
    def extra_repr(self):
        s = ('{in_channels}, {out_channels}, kernel_size={kernel_size}'
             ', stride={stride}')
        if self._w_bit > 0 or self._a_bit > 0:
            s += ', w_bit={}, a_bit={}'.format(self._w_bit, self._a_bit)
            s += ', half wave' if self._half_wave else ', full wave'
...
class QLinearLayer(MyModule):
    ...
    def __init__(self, in_features, out_features, bias=True,
        ...
        if self.dropout_rate > 0:
            ...
            modules['dropout'] = None
        # linear
        modules['weight'] = {'linear': QLinear(self.in_features, self.out_features, self.bias, w_bit=w_bit, a_bit=a_bit, half_wave=half_wave)}
    ...
    def extra_repr(self):
        ...
        if self._w_bit > 0 or self._a_bit > 0:
            s += ', w_bit={w_bit}, a_bit={a_bit}'.format(w_bit=self._w_bit, a_bit=self._a_bit)
            s += ', half wave' if self._half_wave else ', full wave'
        return s
...
class MBInvertedQConvLayer(MyModule):
    ...
    def __init__(self, in_channels, out_channels,
                 kernel_size=3, stride=1, expand_ratio=6, mid_channels=None, act_func='relu6',
        ...
        if self.expand_ratio == 1:
            ...
            self.inverted_bottleneck = nn.Sequential(OrderedDict([
                ('conv', QConv2d(self.in_channels, feature_dim, 1, 1, 0, bias=False, w_bit=pw_w_bit, a_bit=pw_a_bit, half_wave=False)),
                ('bn', nn.BatchNorm2d(feature_dim)),
                ('act', build_activation(self.act_func, inplace=True)),
            ]))
        ...
        pad = get_same_padding(self.kernel_size)
        depth_conv_modules = [
            ('conv', QConv2d(feature_dim, feature_dim, kernel_size, stride, pad, groups=feature_dim, bias=False, w_bit=dw_w_bit, a_bit=dw_a_bit)),
            ('bn', nn.BatchNorm2d(feature_dim)),
            ('act', build_activation(self.act_func, inplace=True))
        ]
        self.depth_conv = nn.Sequential(OrderedDict(depth_conv_modules))
        ...
        self.point_linear = nn.Sequential(OrderedDict([
            ('conv', QConv2d(feature_dim, out_channels, 1, 1, 0, bias=False, w_bit=pw_w_bit, a_bit=pw_a_bit)),
            ('bn', nn.BatchNorm2d(out_channels)),
        ]))
