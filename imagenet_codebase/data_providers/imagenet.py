...
class ImagenetDataProvider(DataProvider):
    ...
    def build_sub_train_loader(self, n_images, batch_size, num_worker=None):
        if self.__dict__.get('sub_train_%d' % self.active_img_size, None) is None:
            ...
            g = torch.Generator()
            g.manual_seed(DataProvider.SUB_SEED)  # set random seed before sampling validation set
...
class ImageNet100DataProvider(ImagenetDataProvider):
    ...
    def save_path(self):
        if self._save_path is None:
            self._save_path = './dataset/imagenet100'
            if not os.path.exists(self._save_path):
                self._save_path = os.path.expanduser('~/dataset/imagenet100')
        return self._save_path
